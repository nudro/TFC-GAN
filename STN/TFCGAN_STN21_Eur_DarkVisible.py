...
"""
For Iris: Remove LPIPS, Just use L1
...
#criterion_GAN_B = torch.nn.MSELoss() # LSGAN Style
...
#criterion_ang = torch.nn.TripletMarginLoss()
#criterion_pixel_loss = nn.L1Loss()
...
class LocalizerVIT(nn.Module):
    def __init__(self, img_shape):
        ...
        self.vit = nn.Sequential(
            K.VisionTransformer(image_size=self.h, patch_size=16, in_channels=channels*2) 
        )
    ...
    def forward(self, img_A, img_B, src):
        ...
        with autocast(): 
            ...
            for i in range(len(src_tensors)):
                rs_grid = F.affine_grid(theta_batches[i], src_tensors[i].size(), align_corners=True) # True
                Rs = F.grid_sample(src_tensors[i], rs_grid,  mode='bicubic', padding_mode='border', align_corners=False)  # bicubic/True
...
class UNetDownNOBP(nn.Module):
    # No Blurpool, makes it regular pix2pix
    def __init__(self, in_size, out_size, normalize=True, dropout=0.0):
        super(UNetDownNOBP, self).__init__()
...
class UNetUpNOBP(nn.Module):
    # No Blurpool, makes it regular pix2pix
    def __init__(self, in_size, out_size, dropout=0.0):
        super(UNetUpNOBP, self).__init__()
        layers = [
            nn.ConvTranspose2d(in_size, out_size, 4, 2, 1, bias=False),
            nn.InstanceNorm2d(out_size),
            nn.ReLU(inplace=True),
...
class Discriminator1(nn.Module):
    # Regular UNET, PIX2PIX, NO BLURPOOL
    def __init__(self, img_shape):
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            # min recon loss
            #recon_lossA = global_pixel_loss(fake_A, real_A) #L1
            recon_loss = global_pixel_loss(warped_B, fake_B)
            #recon_loss = (recon_lossA + recon_lossB).mean()
