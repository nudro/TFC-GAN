...
def mesh_loss(keysA, idA, keysRB, idB, keys_realB, id_realB):
    ...
    keys_realB_f = [i for j, i in enumerate(keys_realB) if j not in indices] # list
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            # real thermal 
            keys_realB, id_realB = face_mesh(real_A)
            
            loss_mesh = mesh_loss(keysA, idA, keysRB, idB, keys_realB, id_realB) # calc loss between both landmarks
