...
from datasets_temp_B import *

"""
Yes, the TFC-GAN could be right. But V6 is not leading to any change in the real_B because all the visible generated
images are in the same geometry, matching B. So I know this means that the tFC-GAN is some kind of key here. Do the
vis-> thr translation then see if it will warp B. 
"""
...
def sample_images(batches_done):
    ...
    fake_B_init = generator(real_A) 
    warped_fB = model(img_A=real_A, img_B=real_B, src=fake_B_init)
    T_ = torch.cat(warped_fB) 

    warped_A = model(img_A=real_A, img_B=real_B, src=real_A)
    R_ = torch.cat(warped_A)
    fake_B_R = generator(R_)
    
    warped_B = model(img_A=real_A, img_B=real_B, src=real_B)
    B = torch.cat(warped_B)
    ...
    warped fake B,
    registered real A,
    fake B from the registered A,
    B,
    registered B
    >
    """
    img_sample_global = torch.cat((real_A.data, T_.data, R_.data, fake_B_R.data, real_B.data, B.data), -2)
...
def structural_adversarial(synth_B, real_A, real_B, B1, B2, B3, B4):

    # Triplet - Structural integrity 
    # triplet loss on the patches of fake_B
    fake_B1 = synth_B[:, :, 0:0+opt.img_width//2, 0:0+opt.img_height//2] #(x,y) = (0,0)
    fake_B2 = synth_B[:, :, 0:0+opt.img_width//2, 128:128+opt.img_height//2] #(x,y) = (0, 128)
    fake_B3 = synth_B[:, :, 128:128+opt.img_width//2, 0:0+opt.img_height//2] #(x,y)=(128,0)
    fake_B4 = synth_B[:, :, 128:128+opt.img_width//2, 128:128+opt.img_height//2] #(x,y) = (128,128)
    ...
    loss_pix_g = criterion_lpips(synth_B, real_B)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            fake_B_init = generator(real_A) 
            patch_init, pix_init = structural_adversarial(fake_B_init, real_A, real_B, B1, B2, B3, B4)
            warped_fB = model(img_A=real_A, img_B=real_B, src=fake_B_init)
            ...
            warped_A = model(img_A=real_A, img_B=real_B, src=real_A)
            R_ = torch.cat(warped_A)
            fake_B_R = generator(R_) # send to D
            patch_B_R, pix_B_R = structural_adversarial(fake_B_R, real_A, real_B, B1, B2, B3, B4)
         
            # Structural - Triplet Loss
            loss_structural = patch_init + patch_B_R
            
            # LPIPS loss - perceptual similarity
            loss_reconstruction = 0.01*(pix_init + pix_B_R)
            
            # Adverarial - How fake and how real
            pred_fake_init = discriminator(T_.cuda(), real_A)
            pred_fake_AR = discriminator(fake_B_R.cuda(), real_A)
        ...
        with autocast():
            ...
            pred_fake_TR_d = discriminator(T_.detach(), real_A)
            pred_fake_RT_d = discriminator(fake_B_R.detach(), real_A)
