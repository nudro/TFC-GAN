...
No Stacked Method - doesn't learn a matrix, fails. 
...
def sample_images(batches_done):
    ...
    B_R = model(img_A=real_A, img_B=fake_B, src=real_B)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.localization = nn.Sequential(
            K.VisionTransformer(image_size=opt.img_height, patch_size=16, in_channels=opt.channels*2) # (A,B), changed from 16-> 32 patch
        )
    ...
    def forward(self, img_A, img_B, src):
        ...
        with autocast():  
            ...
            for i in range(len(src_tensors)):
                ...
                print("rs_grid:", rs_grid.size()) # torch.Size([1, 256, 256, 2])
                print("src_tensors[i]:", src_tensors[i].size()) # torch.Size([1, 3, 256, 256])
                Rs = F.grid_sample(src_tensors[i], rs_grid,  mode='bicubic', padding_mode='border', align_corners=True)  
                print("Rs:", Rs.size()) # torch.Size([1, 3, 256, 256])
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            # // Register//
            BR = model(img_A=real_A, img_B=fake_B, src=real_B)
            ...
            loss_IOU = iou_global_loss(fake_B, BR) 
            ...
            alpha1 = 0.001
            alpha2 = 0.01
            loss_G = loss_GAN + alpha1*loss_IOU + alpha2*loss_pixel
