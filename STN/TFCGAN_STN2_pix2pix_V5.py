...
################
"""      
class UNetDownAA(nn.Module):
    def __init__(self, in_size, out_size, normalize=True, dropout=0.0):
        super(UNetDownAA, self).__init__()
...
class UNetUpAA(nn.Module):
    def __init__(self, in_size, out_size, dropout=0.0):
        super(UNetUpAA, self).__init__()
        layers = [
...
class LocalizerUNet(nn.Module):
    def __init__(self, img_shape):
        ...
        self.down1 = UNetDownAA(channels*2, 64)
        self.down2 = UNetDownAA(64, 128)
        self.down3 = UNetDownAA(128, 256)
        self.down4 = UNetDownAA(256, 512)
        self.down5 = UNetDownAA(512, 512)
        self.down6 = UNetDownAA(512, 512)

        self.up1 = UNetUpAA(512, 512)
        self.up2 = UNetUpAA(1024, 512)        
        self.up3 = UNetUpAA(1024, 256)
        self.up4 = UNetUpAA(512, 128)
        self.up5 = UNetUpAA(256, 64) # Then it'll only be 64 x 64
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, LPIPS: %f, GAN: %f] ETA: %s"
            % (
                epoch, #%d
                opt.n_epochs, #%d
                i, #%d
