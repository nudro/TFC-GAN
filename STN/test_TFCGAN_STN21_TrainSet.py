...
"""
Runs the STN on the training data. Goal is to retrain the model on the newly registered images. Assumption is that
the model will have an easier time with less abrupt changes.  But you have to still pair them using a pair
script like from pix2pix.
...
class LocalizerVIT(nn.Module):
    ...
    def forward(self, x):
        out = self.vit(x) # returns torch.Size([batch, 257, 768])
        return out
...
dataloader = DataLoader(
    ...
    batch_size=1, # 1 image at a time, will be entire training set
    shuffle=False, # needs to be in order
    num_workers=4,
    drop_last=True,
)
