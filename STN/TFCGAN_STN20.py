...
def sample_images(batches_done):
    ...
    #img_deformation = torch.cat((plot_init), -1)
    save_image(img_sample_global, "images/%s/%s.png" % (opt.experiment, batches_done), nrow=4, normalize=True)
    #save_image(img_deformation, "images/%s/%s_grid.png" % (opt.experiment, batches_done), nrow=4, normalize=True)
    ...
    def forward(self, img_A, img_B, src):
        ...
        with autocast(): 
            ...
            for i in range(len(src_tensors)):
                ...
                Rs = F.grid_sample(src_tensors[i], rs_grid,  mode='bicubic', padding_mode='border', align_corners=True)  # I changed from border, I wnat to see the change
...
generator1 = torch.nn.DataParallel(generator1, device_ids=[0, 1, 2])
generator2 = torch.nn.DataParallel(generator2, device_ids=[0, 1, 2])
discriminator1 = torch.nn.DataParallel(discriminator1, device_ids=[0, 1, 2])
discriminator2 = torch.nn.DataParallel(discriminator2, device_ids=[0, 1, 2])
model = torch.nn.DataParallel(model, device_ids=[0, 1, 2])  


if opt.epoch != 0:
    # Load pretrained models
    generator1.load_state_dict(torch.load("/home/local/AD/cordun1/experiments/TFC-GAN/saved_models/%s/generator1_%d.pth" % (opt.experiment, opt.epoch)))
    generator2.load_state_dict(torch.load("/home/local/AD/cordun1/experiments/TFC-GAN/saved_models/%s/generator2_%d.pth" % (opt.experiment, opt.epoch)))
    ...
    discriminator2.load_state_dict(torch.load("/home/local/AD/cordun1/experiments/TFC-GAN/saved_models/%s/discriminator2_%d.pth" % (opt.experiment, opt.epoch)))
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            # pass to generator 2 for fake_A
            warped_B = model(img_A=real_A, img_B=fake_B, src=real_B) 
            fake_A = generator2(warped_B.detach())
            
            # min recon loss
            recon_loss = global_pixel_loss(fake_A, real_A) # L1
            
            # perceptual loss
            L1 = criterion_lpips(fake_A, real_A)
            ...
            alpha1 = 0.001
            alpha2 = 0.01
            loss_G = loss_GAN + alpha2*recon_loss + L1
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, R: %f, L1: %f] ETA: %s"
            % (
                epoch, #%d
                opt.n_epochs, #%d
                i, #%d
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(generator1.state_dict(), "/home/local/AD/cordun1/experiments/TFC-GAN/saved_models/%s/generator1_%d.pth" % (opt.experiment, epoch))
        torch.save(generator2.state_dict(), "/home/local/AD/cordun1/experiments/TFC-GAN/saved_models/%s/generator2_%d.pth" % (opt.experiment, epoch))
        ...
        torch.save(discriminator2.state_dict(), "/home/local/AD/cordun1/experiments/TFC-GAN/saved_models/%s/discriminator2_%d.pth" % (opt.experiment, epoch))
