...
from datasets_temp_020522 import *
...
02/05/22 
Uses a temperature "map": where I matmul the raw pixels x temps
and minimize this dot product
...
lambda_t = 10
criterion_temp = torch.nn.L1Loss()
...
def vectorize_temps(fake_B):
    ...
    for t in range(0, opt.batch_size):
        ...
        TFB.append(tfb) #temps of fake_B for the whole batch    
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            # // Temperature loss here//
            # raw pixels and temps of fake_B
            TFB_ = vectorize_temps(fake_B)
            # reshape real_B's temps
            TB = TB.reshape(TB.size(0), 1, TB.size(1), TB.size(2))
            
            # multiply matmul - image's R channel
            B_map = real_B[:,0,:,:].matmul(TB)
            Bfake_map = fake_B[:,0,:,:].matmul(TFB_)
            loss_temp_g = criterion_temp(B_map, Bfake_map)/1000 #scaling otherwise will be like 3032.10
