...
# NOTE - I JUST CREATED A NEW EA_TEST_UPDATED Dataset
# No need any more to use the complicated dataloader I created below.
# For the test script, just use the EA_test_updated dataset
...
def sample_images(batches_done):
    ...
    real_A_1 = Variable(imgs["A_1"].type(HalfTensor))
    real_B_1 = Variable(imgs["B_1"].type(HalfTensor))
    fake_B_1 = generator(real_A_1)
    
    real_A_2 = Variable(imgs["A_2"].type(HalfTensor))
    real_B_2 = Variable(imgs["B_2"].type(HalfTensor))
    fake_B_2 = generator(real_A_2)
    
    """"
    # Too much right now - do later at test time
    ...
    # GLOBAL
    img_sample_global_1 = torch.cat((real_A_1.data, fake_B_1.data, real_B_1.data), -2)
    save_image(img_sample_global_1, "images/%s/%s_g.png" % (opt.experiment, batches_done), nrow=5, normalize=True)
    
    img_sample_global_2 = torch.cat((real_A_2.data, fake_B_2.data, real_B_2.data), -2)
    save_image(img_sample_global_2, "images/%s/%s_g.png" % (opt.experiment, batches_done), nrow=5, normalize=True)
...
test_dataloader = DataLoader(
    TestDataset(root_dir1 = "/home/local/AD/cordun1/experiments/data/eurecom_v3_pairs",
                root_dir2 = "/home/local/AD/cordun1/experiments/data/ADAS_fullset",
                 transforms_=transforms_,
                mode="test"), 
    batch_size=1*2,
    shuffle=True,
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch1 in enumerate(eur_dataloader): 
        ...
        B1 = concat_output("B1", idx)
        B2 = concat_output("B2", idx)
        B3 = concat_output("B3", idx)
        B4 = concat_output("B4", idx)
