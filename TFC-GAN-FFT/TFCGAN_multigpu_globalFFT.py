...
#triplet_loss.cuda() # This version does not use patch-baed losses
criterion_temp.cuda()
criterion_amp.cuda()
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            loss_G = 0.5*loss_GAN_g + 0.5*loss_pix_g + 0.5*loss_temp_g + 1/100*loss_FFT
        ...
        # Print log
        sys.stdout.write(
            "\r |Experiment: %s| [Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f | GAN G: %f | pix_G: %f | temp_G: %f | fft: %f] ETA: %s"
            % (
                opt.experiment, 
                epoch, #%d
                opt.n_epochs, #%d
