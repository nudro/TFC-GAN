...
May 4 - Also adds KL instead of L1 loss for FFT
What will happen?
...
criterion_amp_R = nn.KLDivLoss(reduction="mean", log_target=True) # Regional Amp
criterion_phase_R = nn.KLDivLoss(reduction="mean", log_target=True) # Regional Phase
criterion_amp_P = nn.L1Loss() # Patch Amp
criterion_phase_P = nn.L1Loss() # Patch Phase
...
def fft_loss(fake_B, B1, B2, B3, B4):
    ...
    # L1 loss
    loss_Amp = criterion_amp_P(A1f, A1r) + criterion_amp_P(A2f, A2r) + criterion_amp_P(A3f, A3r) + criterion_amp_P(A4f, A4r)
    loss_Pha = criterion_phase_P(P1f, P1r) + criterion_phase_P(P2f, P2r) + criterion_phase_P(P3f, P3r) + criterion_phase_P(P4f, P4r)
...
#####################
def regional_fft_loss(fake_B, real_B):
    """
    The hair and eye regions, each 256 x 128
    return a 100 x 129 amplitude and phase array, each.
    """
    def reg_fft(thermal_tensor):
        AMP = []
        PHA = []
        for t in range(0, opt.batch_size): 
            # thermal images must be in grayscale (1 channel)
            ...
            fft_space = FFT_Components(b)
            amp, phase = torch.Tensor(fft_space.make_components()).cuda() # convert them into torch tensors
            #print("******Shape of amp tensor:", amp.size()) # torch.Size([100, 129])
            AMP.append(amp)
            PHA.append(phase)
        AMP_tensor = torch.cat(AMP).reshape(opt.batch_size, 1, 100, 129)    
        PHA_tensor = torch.cat(PHA).reshape(opt.batch_size, 1, 100, 129)    
        return AMP_tensor, PHA_tensor
    
    
    # fake hair and eyes features
    hair_ = fake_B[:, :, 0:100, 0:0+opt.img_width] # torch.Size([batch, 3, 100, 256])
    eyes_ = fake_B[:, :, 100:opt.img_width-56, 0:0+opt.img_width] # torch.Size([batch, 3, 100, 256])
    
    # real hair and eyes features
    hair = real_B[:, :, 0:100, 0:0+opt.img_width] # torch.Size([batch, 3, 100, 256])
    eyes = real_B[:, :, 100:opt.img_width-56, 0:0+opt.img_width] # torch.Size([batch, 3, 100, 256])
    
    # Fourier Transform Loss for Each Patch
    # >> fake
    Ah_F, Ph_F = reg_fft(hair_) 
    Ae_F, Pe_F = reg_fft(eyes_) 
    
    # >> real
    Ah_R, Ph_R = reg_fft(hair) 
    Ae_R, Pe_R = reg_fft(eyes) 
    
    #print("Raw sample inputs before log:/n", Ae_F)
    #print("Raw targets before log:/n", Ae_R)
    
    # FOR KL DIV, convert to log space dist
    # I think I want the log across the batch
    Ah_F, Ph_F = F.log_softmax(Ah_F, dim=0), F.log_softmax(Ph_F, dim=0) #inputs
    Ah_R, Ph_R = F.log_softmax(Ah_R, dim=0), F.log_softmax(Ah_R, dim=0) #target (real)
    
    Ae_F, Pe_F = F.log_softmax(Ae_F, dim=0), F.log_softmax(Pe_F, dim=0) #inputs
    Ae_R, Pe_R = F.log_softmax(Ae_R, dim=0), F.log_softmax(Ae_R, dim=0) #target (real)
    
    #print("sample inputs:/n", Ae_F)
    #print("sample target:/n", Ae_R)
    
    # KL Div loss
    Amp_h = criterion_amp_R(Ah_F, Ah_R)
    Amp_e = criterion_amp_R(Ae_F, Ae_R)
    
    Pha_h = criterion_phase_R(Ph_F, Ph_R)
    Pha_e = criterion_phase_R(Pe_F, Pe_R)
    
    loss_Amp_reg = Amp_h + Amp_e
    loss_Pha_reg = Pha_h + Pha_e
    loss_FFT_reg = 1/2*(loss_Amp_reg + loss_Pha_reg)
...
triplet_loss.cuda()
criterion_temp.cuda()
criterion_amp_R.cuda()
criterion_phase_R.cuda()
criterion_amp_P.cuda()
criterion_phase_P.cuda()
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            #>>FFT Patch Loss
            loss_FFT = 0.001*fft_loss(fake_B, B1, B2, B3, B4)
            
            # >>FFT Regional 
            loss_FFT_reg = 0.01*regional_fft_loss(fake_B, real_B)
            ...
            loss_pix_g = 0.10*criterion_lpips(fake_B, real_B)

            #>>Total Generator Loss
            #[D loss: 0.406331] [G loss: 13.071980 | GAN G: 1.118602 | pix_G: 14.506828 | patch: 0.407370 | temp_G: 9.928969 | loss_FFT_reg: 1821.900146]
            loss_G = 1/2*(loss_GAN_g + 0.0001*loss_FFT_reg + loss_patch + loss_temp_g + loss_pix_g)
        ...
        # Print log
        sys.stdout.write(
            "\r |Experiment: %s| [Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f | GAN G: %f | pix_G: %f | patch: %f | temp_G: %f | patch_FFT: %f | loss_FFT_reg: %f] ETA: %s"
            % (
                opt.experiment, 
                epoch, #%d
                opt.n_epochs, #%d
