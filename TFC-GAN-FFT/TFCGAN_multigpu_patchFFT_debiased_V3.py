...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            # weigh the ethnicity loss the highest, penalize it the most
            loss_label = criterion_label(gen_f, gender) + 10*criterion_label(eth_f, ethn) + criterion_label(age_f, age)
        ...
        with autocast():
            ...
            print("Predicted Gender: {} vs Real Gender: {} | Fake Thermal:".format(torch.argmax(pred_fake_gen[:3]), gender[:3]))
            print("Predicted Ethnicity: {} vs Real Ethn: {} | Fake Thermal:".format(torch.argmax(pred_fake_ethn[:3]), ethn[:3]))
            print("Predicted Age: {} vs Real Age: {} | Fake Thermal:".format(torch.argmax(pred_fake_age[:3]), age[:3]))
