...
"""
B2A for Eurecom 
Conventional Eurecom model fails in opposite direction.
So trying this. 
...
def sample_images(batches_done):
    ...
    real_A = Variable(imgs["B"].type(HalfTensor)) # torch.Size([1, 3, 256, 256])
    real_B = Variable(imgs["A"].type(HalfTensor))
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        real_A = Variable(batch["B"].type(HalfTensor))
        real_B = Variable(batch["A"].type(HalfTensor))
