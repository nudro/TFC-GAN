...
Resize the ViT output to 256 x 256. 
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            # pass to generator 2 for fake_A
            warped_B = model(img_A=real_A, img_B=fake_B, src=real_B) 
            fake_A = generator2(warped_B) # don't detach, otherwise, the G won't get updates, nor will the STN, but then it'll go black??
            
            # min recon loss
            recon_loss = global_pixel_loss(fake_A, real_A) # L1
            #recon_loss2 = global_pixel_loss(real_B, warped_B) # L1
            ...
            alpha1 = 0.001
            alpha2 = 0.01
            loss_G = loss_GAN + alpha2*recon_loss + perc_loss
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, R: %f, LPIPS: %f] ETA: %s"
            % (
                epoch, #%d
                opt.n_epochs, #%d
                i, #%d
