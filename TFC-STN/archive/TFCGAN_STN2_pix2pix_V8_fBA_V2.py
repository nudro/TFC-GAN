...
criterion_mesh = torch.nn.MSELoss()
...
def sample_images(batches_done):
    ...
    >
    """
    img_sample_global = torch.cat((real_A.data, real_B.data, fake_A.data, fake_B.data, R_.data), -2)
...
def face_mesh(face_tensor, face_mesh=my_face_mesh):
    ...
    for m in range(0, face_tensor.size(0)): # use face_tensor.size(0) b/c batch size is spread over multiple GPUs
        ...
        face_norm = cv2.normalize(face, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F) # ensure dtype is the same as array going in
...
def mesh_loss(keysA, idA, keysRB, idB):
    """
    ...
    indices = list(set(idA).union(idB))

    # have to do all at once
    keysA_f = [i for j, i in enumerate(keysA) if j not in indices] # list
    keysRB_f = [i for j, i in enumerate(keysRB) if j not in indices] # list
    
    # convert list of arrays to torch tensors 
    RB = key_maker(keysA_f) # extract all values
    RB_tensor = torch.from_numpy(RB)
    
    AK = key_maker(keysRB_f)
    A_tensor = torch.from_numpy(AK)
    
    loss = criterion_mesh(RB_tensor, A_tensor)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast(): 
            ...
            print("Generate fake_B")
            fake_B = generator(real_A) 
            loss_triplet_patch = triplet_patches(fake_B, B1, B2, B3, B4)
            # calc LPIPS
            loss_pixelB = criterion_lpips(fake_B, real_B)
            
            print("Register real_B with fBA, generate fake_A")
            warped_B = model(img_A=fake_B, img_B=real_A, src=real_B)
            R_ = torch.cat(warped_B)
            fake_A = generator(R_)
            
            loss_pixelA = criterion_lpips(fake_A, real_A)
            loss_pixel = 0.5*(loss_pixelB + loss_pixelA)
            
            #Face Mesh Loss
            keysA, idA = face_mesh(real_A) # get hte keypoints for the real A
            keysRB, idB = face_mesh(R_) # get the keypoints for the regisetered B
            
            loss_mesh = mesh_loss(keysA, idA, keysRB, idB) # calc loss between both landmarks
            ...
            loss_GAN = criterion_GAN(pred_fake_B - real_pred.detach(), valid) + criterion_GAN(pred_fake_A - real_pred.detach(), valid)
        
            # Total Loss
            loss_G = loss_GAN + loss_triplet_patch + 1/10*loss_pixel + 10*loss_mesh
        ...
        with autocast():
            ...
            #fake 
            pred_fake_B_d = discriminator(fake_B.detach(), real_A)
