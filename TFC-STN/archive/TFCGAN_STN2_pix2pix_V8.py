...
def sample_images(batches_done):
    ...
    >
    """
    img_sample_global = torch.cat((real_A.data, real_B.data, fake_A.data, fake_B.data, T_.data, R_.data), -2)
...
"""    
class LocalizerUNet(nn.Module):
    #This version uses the BlurPool UNET Down and Up MOdules
    def __init__(self, img_shape):
        super(LocalizerUNet, self).__init__()
        channels, self.h, self.w = img_shape
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            ...
            loss_fake = criterion_GAN(pred_fake_B_d, fake) + criterion_GAN(pred_fake_A_d, fake)
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, fft: %f, trip: %f, GAN: %f | pix: %f] ETA: %s"
            % (
                epoch, #%d
                opt.n_epochs, #%d
                i, #%d
