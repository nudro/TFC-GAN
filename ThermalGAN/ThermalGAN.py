...
class UNetDown_bic(nn.Module):
    def __init__(self, in_size, out_size, normalize=True, dropout=0.0):
        ...
        if normalize:
            layers.append(nn.InstanceNorm2d(out_size)) # was Batch
...
class MultiDiscriminator(nn.Module):
    """
    #This discriminator has different dimensions at each module in the D:
    #exa: [32, 1, 16, 16] at entry 0 and [32, 1, 8, 8] at entry 1
    
    #This means you have to calculate the loss inside this class
    """
    def __init__(self, img_shape):
        ...
        def discriminator_block(in_filters, out_filters, normalize=True):
            ...
            if normalize:
                layers.append(nn.InstanceNorm2d(out_filters, 0.8)) # Batch
            layers.append(nn.LeakyReLU(0.2))
            return layers
    ...
    def forward(self, x, gt):
        """
        #You have to calculate the loss inside this function, since each is a different diemnsion:
        #MSE(outputs[0], 1), etc. 
        """
        with autocast():
            outputs = []
            for m in self.models:
                ...
                x = self.downsample(x)
                #print("x size:", x.size())
            #print("Multidiscrim debugging")
            #print("([torch.mean((out - gt) ** 2) for out in outputs]):", ([torch.mean((out - gt) ** 2) for out in outputs]))
            
            """
            for out in outputs: 
                print("out:", out[0,0,0,0])
                print("out - gt:", out-gt)
                print("out - gt **2:", (out-gt)**2)
            ...
            for out in outputs:
                if gt==1: 
                    #print("gt==1:", 1)
                    #gt_ = torch.ones(out.size(0), 1, out.size(2), out.size(3)).type(HalfTensor).cuda()
                    gt_ = torch.ones(out.size(0), 1, out.size(2), out.size(3)).type(Tensor).cuda()
                else:
                    #print("gt==0:", 0)
                    #gt_ = torch.zeros(out.size(0), 1, out.size(2), out.size(3)).type(HalfTensor).cuda()
                    gt_ = torch.zeros(out.size(0), 1, out.size(2), out.size(3)).type(Tensor).cuda()
            ...
            """
            alt: tensor(0.0099, device='cuda:0', grad_fn=<L1LossBackward>)
            at: tensor([0.0073, 0.0051, 0.0099], device='cuda:0', dtype=torch.float16)
            at.mean(): tensor(0.0074, device='cuda:0', dtype=torch.float16)
            """
            #at = (torch.tensor(a).type(HalfTensor)).mean().requires_grad_(requires_grad=True).cuda()
            at = (torch.tensor(a)).mean().requires_grad_(requires_grad=True).cuda()

            #print("at:", at) # at: tensor([1.0073, 1.0051, 0.9901]); therefore yoiu have to take the mean
            #print("----------------")
               
        return at # returns a list
...
def sample_images(batches_done):
    ...
    fake_B = generator_G2(fake_S).cuda()
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        scaler.scale(vae_real).backward() # retain_graph=True
        scaler.unscale_(optimizer_D_VAE)
        ...
        # ---------------------------------
        print("+ + + optimizer_D_LR.zero_grad() + + +")
        optimizer_D_LR.zero_grad()
        with autocast():
            lr_real = (D_LR(real_S, valid_G1)).mean()
            lr_fake = (D_LR(fake_S.detach(), fake_G1)).mean()
            loss_D_LR = lr_real + lr_fake
            print("===loss_D_LR:", loss_D_LR)
        
        scaler.scale(loss_D_LR).backward() # retain_graph=True
        scaler.step(optimizer_D_LR)
        print("+ + + optimizer_D_LR.step() + + +")
        """
 
        """ 
        Stage 2: pix2pix - G2 
        GOAL: To generate R^ (the thermal fake_B), given fake_S
        """
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [loss_VAE_GAN: %f, loss_kl: %f, loss_pixel_bic: %f, loss_latent: %f, loss_GAN: %f, loss_pixel_pix: %f, loss_G: %f, loss_D: %f, loss_real_d: %f, loss_fake_d: %f] ETA: %s"
            % (
                epoch, #%d
                opt.n_epochs, #%d
                i, #%d
