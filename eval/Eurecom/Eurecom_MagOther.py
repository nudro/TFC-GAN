...
def other_spec(master_array):
    ...
    for i in range(0, len(master_array)):  # len is the same for fake and real
        ...
        try: 
            ...
            score = mean_absolute_error(real_mag, fake_mag)
        except:
            continue
...
def make_spectra(image, i, mode, experiment):
    ...
    matplotlib.image.imsave("/home/local/AD/cordun1/experiments/TFC-GAN/evaluation/Eurecom/{}/spectra/{}_{}.png".format(experiment, i, mode), magnitude_spectrum)
...
def main(fake_dir, real_dir, experiment, test_set):
    ...
    # calculate values
    other_values, other_table = other_spec(master)
    other_values_ = np.array(other_values)

    # Return average
    print("--------------Experiment: {} | Average Score of Magnitude Spectra: [ {} ]-------------".format(experiment, other_values_.mean()))

    """
    # Save spectra images
    save_spectra_images(master, experiment)
    
    eurecom_test_files = pd.read_csv(opt.test_set+".txt", sep=" ", header=None)
    b = eurecom_test_files[0] # images 0 through 105 in the fake and real dirs follow exactly the order of the test files
    other_table = pd.concat([other_table, b], axis=1)
    
    # don't save it
    #other_table.to_csv('/home/local/AD/cordun1/experiments/TFC-GAN/evaluation/Eurecom/%s/mse_spec.csv' % (experiment))
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test_set", type=str, default="eurecom_test_set", help="list of test set file names")
    ...
    print("This is a script to test other metrics, no spectra are saved..")

    #os.makedirs("/home/local/AD/cordun1/experiments/TFC-GAN/evaluation/Eurecom/%s/spectra/" % opt.experiment, exist_ok=True)
